...
namespace Dimeng.LinkToMicrocad
{
    public class AKInfo
    {
        ...
        public static AKInfo GetInfo()
        {
            try
            {
                Logging.Logger.GetLogger().Info("Getting current product information from registry.");
                ...
                Logging.Logger.GetLogger().Info(string.Format("Path:{0}", akInfo.Path));

                akInfo.Country = akr14.GetValue("Country").ToString();
                Logging.Logger.GetLogger().Info(string.Format("Country:{0}", akInfo.Country));

                akInfo.MeterToUnit = akr14.GetValue("MeterToUnit").ToString();
                Logging.Logger.GetLogger().Info(string.Format("MeterToUnit:{0}", akInfo.MeterToUnit));

                akInfo.SerialNumber = akr14.GetValue("SerialNumber").ToString();
                Logging.Logger.GetLogger().Info(string.Format("SerialNumber:{0}", akInfo.SerialNumber));

                Logging.Logger.GetLogger().Info("Registry reading finished.");
