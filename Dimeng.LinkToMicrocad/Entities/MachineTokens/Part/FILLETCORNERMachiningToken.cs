...
{
    public class FILLETCORNERMachiningToken : UnAssociativeToken
    {
        public FILLETCORNERMachiningToken(string token, string par1, string par2, string par3, string par4, string par5, string par6, string par7, string par8, string par9)
        ...
        {
            this.FaceNumber = check.FaceNumber(this.Token, 12, new int[] { 1, 2, 3, 4, 5, 6, 7, 8 }, this.Errors);
            this.EdgeNumber = check.EdgeNumber(this.Token, 14, new int[] { 1, 2, 3, 4, 5, 6, 7, 8 }, this.Errors);
            Radius = check.GetDoubleValue(Par1, "FILLET/半径", true, this.Errors);
            Depth = check.GetDoubleValue(Par3, "FILLET/深度", true, this.Errors);
            LeadIn = check.GetDoubleValue(Par4, "FILLET/进退刀距离", true, this.Errors);
            ToolName = check.ToolName(Par7, "FILLET/刀具名称", this.Errors);
            IsDrawOnly = check.GetBoolValue(Par8, "FILLET/仅用于绘图", false, false, this.Errors);
