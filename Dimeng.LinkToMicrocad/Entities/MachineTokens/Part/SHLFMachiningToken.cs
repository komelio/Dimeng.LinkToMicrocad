...
namespace Dimeng.WoodEngine.Entities.MachineTokens
{
    public class SHLFMachiningToken : AssociativeToken
    {
        ...
        public override bool Valid(MachineTokenChecker check)
        {
            ...
            DistToBottom = check.GetDoubleValue(this.Par1, "关联的层板孔（类型1）/到底部距离", true, this.Errors);
            DistToFirstHole = check.GetDoubleValue(this.Par2, "关联的层板孔（类型1）/至第1排孔的距离", false, this.Errors);
            FaceHoleDepth = check.GetDoubleValue(this.Par3, "关联的层板孔（类型1）/面孔深度", true, this.Errors);
            DistToTop = check.GetDoubleValue(this.Par4, "关联的层板孔（类型1）/至顶部的距离", true, this.Errors);
            DistToSecondHole = check.GetDoubleValue(this.Par5, "关联的层板孔（类型1）/至第二排孔的距离", false, this.Errors);
            FaceHoleDiameter = check.GetDoubleValue(this.Par6, "关联的层板孔（类型1）/面孔直径", true, this.Errors);
            DistBetweenTwoHoles = string.IsNullOrEmpty(this.Par7) ? 32 : check.GetDoubleValue(this.Par7, "关联的层板孔（类型1）/面孔直径", true, this.Errors);
            GapDist = check.GetDoubleValue(Par8, "关联的层板孔（类型1）/两孔距离", true, this.Errors);
            IsDrillFromOpposite = check.GetBoolValue(this.Par9, "关联的层板孔（类型1）/反方向", false, false, this.Errors);//反向的
