...
{
    public class FILLETCORNERMachiningToken : BaseToken
    {
        public FILLETCORNERMachiningToken(string token, string par1, string par2, string par3, string par4, string par5, string par6, string par7, string par8, string par9, int row, int column, Part p)
        ...
        public override bool Valid(Logger logger)
        {
            ...
            base.faceNumberChecker(this.Token, 12, new int[] { 1, 2, 3, 4, 5, 6, 7, 8 });
            base.edgeNumberChecker(this.Token, 14, new int[] { 1, 2, 3, 4, 5, 6, 7, 8 });
            Radius = base.DoubleChecker(Par1, "FILLET/半径", true);
            Depth = base.DoubleChecker(Par3, "FILLET/深度", true);
            LeadIn = base.DoubleChecker(Par4, "FILLET/进退刀距离", true);
            ToolName = base.notEmptyStringChecker(Par7, "FILLET/刀具名称");
            IsDrawOnly = base.BoolChecker(Par8, "FILLET/仅用于绘图", false, false);
